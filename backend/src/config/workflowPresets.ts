...
// admin only picks checkpoint / LoRAs / prompts instead of authoring raw
// workflow JSON. Checkpoint names baked into templates are placeholders —
// patchWorkflow() overrides them from the style at generation time.
//
// Output nodes are PreviewImage (ComfyUI temp/, wiped on its restart), not
// SaveImage: the worker downloads the result in the same job and S3 is the
// archive, so nothing should pile up on the ComfyUI host's disk.
...
export const WORKFLOW_SDXL_STANDARD: Record<string, unknown> = {
  ...
  '7': { inputs: { images: ['6', 0] }, class_type: 'PreviewImage', _meta: { title: 'Preview Image' } },
};
