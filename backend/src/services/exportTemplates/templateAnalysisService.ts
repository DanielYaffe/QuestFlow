...
Use fieldHints to explain the meaning of individual fields when the parser structure alone is not enough. A useful field hint explains what the field represents in gameplay, whether it should be generated by AI, copied from graph/node data, manually filled by the user, treated as a stable identifier, left empty, or preserved as a control flag. Field hints are especially useful for text shown to the player, ids, quantities, dates, booleans, status/control fields, and nested list item properties.

Use relationshipHints to explain how two detected fields relate to each other. A relationship can mean:
- one field stores an id/key/name that references another field's id/key/name,
- one field points to the next, previous, success, failure, accept, decline, or other branch target in the same list or object,
- one field controls whether another field is used, active, terminal, repeatable, hidden, or optional,
- one group/list represents ordered steps where one item's value should be consistent with neighboring items,
- one field classifies or changes how another field should be interpreted.

Relationship hints should describe links that help generation keep the exported data coherent. They are optional: include them only when the detected fields or user examples make the connection reasonably clear. Do not assume a specific game template or hard-code field names. Prefer exact parser paths, including array item paths such as "someList[].targetId" to "someList[].id", when those paths exist.
