  ...
  return (
    <div className="bg-steel-850 border border-steel-700 rounded-md p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-steel-100 text-2xl mb-1">Hero's Journey</h2>
          <p className="text-steel-400 text-sm">Your path to becoming a legendary warrior</p>
        </div>
        <div className="text-right">
          <div className="text-steel-400 text-sm mb-1">Progress</div>
          <div className="text-steel-100 text-2xl">{completedQuests.length}/{questsData.length}</div>
        </div>
      </div>
          ...
          return (
            <div key={quest.id} className="relative">
              {index < questsData.length - 1 && (
                <div className="absolute left-5 top-12 w-0.5 h-16 bg-steel-800" />
              )}
              <div
                className={`relative flex gap-4 p-4 rounded-lg border-2 transition-all ${
                  isCompleted
                    ...
                    : isActive
                    ? 'bg-steel-800 border-pulse hover:bg-steel-700'
                    : 'bg-steel-850/50 border-steel-700 opacity-60'
                }`}
              >
                <div className="flex-shrink-0 mt-1">
                  {isCompleted ? (
                    ...
                    <Circle className="w-10 h-10 text-pulse animate-pulse" />
                  ) : (
                    <Lock className="w-10 h-10 text-steel-500" />
                  )}
                </div>
                <div className="flex-1">
                  <div className="flex items-start justify-between mb-2">
                    <h3
                      className={`text-lg ${
                        isCompleted ? 'text-green-400' : isActive ? 'text-pulse' : 'text-steel-400'
                      }`}
                    >
                      {quest.name}
                    </h3>
                  ...
                  <p className={`text-sm ${isLocked ? 'text-steel-500' : 'text-steel-400'}`}>
                    {quest.description}
                  </p>
                  {isActive && (
                    <div className="mt-3">
                      ...
                      <div className="w-full bg-steel-800 rounded-full h-2">
                        <div className="bg-volt h-2 rounded-full" style={{ width: '60%' }} />
                      </div>
                    </div>
                  )}
                </div>
      ...
      <div className="mt-6 p-4 bg-steel-850 border border-steel-600 rounded-lg">
        <div className="flex items-center justify-between">
          <div>
            <h4 className="text-pulse mb-1">Continue Your Journey</h4>
            <p className="text-steel-400 text-sm">Build your quest in the Quest Builder</p>
          </div>
          <button
            className="px-4 py-2 bg-volt hover:brightness-95 text-steel-950 font-semibold rounded-lg transition-colors text-sm"
